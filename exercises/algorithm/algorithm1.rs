...
impl<T: PartialOrd + Clone + Clone> Default for LinkedList<T> {
    fn default() -> Self {
        Self::new()
    }
}

fn compare<T: PartialOrd + Clone>(a: &T, b: &T) -> bool {
    a > b
}
    ...
    pub fn merge(list_a: LinkedList<T>, list_b: LinkedList<T>) -> Self {
        ...
        loop {
            if j > list_b.length - 1 {
                for i in i..=list_a.length - 1 {
                    result.add((*list_a.get(i as i32).unwrap()).clone());
                }
                break;
            }
            if i > list_a.length - 1 {
                for j in j..=list_b.length - 1 {
                    result.add((*list_b.get(j as i32).unwrap()).clone());
                }
                break;
            }
